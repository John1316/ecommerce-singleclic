  ...
  const renderRatingStars = () => {
    ...
    <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
        <div className="h-56 w-full">
          <Link aria-label={`product_${product.title}`}  to={`/product/${product?.id}`}>
        ...
        <div className="pt-6">
          ...
          <Link aria-label={`products_${product.title}`}  to={`/product/${product?.id}`} className="text-lg font-semibold leading-tight text-gray-900 hover:underline dark:text-white line-clamp-2 min-h-[45px]">{product?.title || ""}</Link>
